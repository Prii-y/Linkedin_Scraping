...
process_data_for_correction <- function(csv_filename) {
  ...
  data <- read_csv(csv_filename) %>%
    ...
    mutate(
      ...
      Last_Name = sub(",.*", "", Last_Name),  # Remove everything after a comma in the name
      incorrect_name = ifelse(
        grepl("\\.", First_Name) | grepl("\\.", Last_Name) | 
          nchar(First_Name) == 1 | nchar(Last_Name) == 1 | is.na(username), 1, 0
      )
    )
  
  # Generate name data to be corrected manually
  name_corr <- data %>% 
    select(First_Name, Last_Name, `Profile Name`, username)
